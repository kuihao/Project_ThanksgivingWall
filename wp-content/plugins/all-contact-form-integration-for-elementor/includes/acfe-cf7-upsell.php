...
class acfe_Upsell {
    ...
    public function init_hooks() {
        ...
        if ( function_exists( 'acfe_activate' ) && function_exists( 'wpcf7' ) ) {
            return;
        }
        ...
        add_action( 'wp_ajax_acfe_cf7_upsell_installer', array( $this, 'install_acfe' ) );
        add_action( 'wp_ajax_acfe_cf7_installer', array( $this, 'install_acfe_cf7' ) );
    }
    /**
    ...
    public function activation_notice() {
        ...
        <script type="text/javascript">
            jQuery(document).ready( function($) {
                $('#acfe-cf-7-install-now').on('click', function (e) {
                    var self = $(this);
                    e.preventDefault();
                    ...
                    $.ajax({
                        ...
                        type: 'post',
                        data: {
                            action: 'acfe_cf7_upsell_installer',
                            _wpnonce: '<?php echo wp_create_nonce('acfe_cf7_upsell_installer'); ?>'
                        },
                ...
                $('#acfe-install-cf7').on('click', function (e) {
                    var self = $(this);
                    e.preventDefault();
                    self.addClass('install-now updating-message');
                    self.text('<?php echo esc_js( 'Installing...' ); ?>');
                    ...
                    $.ajax({
                        ...
                        type: 'post',
                        data: {
                            action: 'acfe_cf7_installer',
                            _wpnonce: '<?php echo wp_create_nonce('acfe_cf7_installer'); ?>'
                        },
                        success: function(response) {
                            self.text('<?php echo esc_js( 'Installed' ); ?>');
    ...
    public function install_acfe() {
        check_ajax_referer( 'acfe_cf7_upsell_installer' );
     ...
     * @return void
     */
    public function install_acfe_cf7() {
        check_ajax_referer( 'acfe_cf7_installer' );

        if ( ! current_user_can( 'manage_options' ) ) {
            wp_send_json_error( __( 'You don\'t have permission to install the plugins' ) );
        }

        $acfe_status = $this->install_acfe_cf7_plugin( 'contact-form-7', 'wp-contact-form-7.php' );
        $this->fail_on_error( $acfe_status );
     ...
     * @return WP_Error|null
     */
    public function install_acfe_cf7_plugin( $slug, $file ) {
        include_once ABSPATH . 'wp-admin/includes/plugin-install.php';
        include_once ABSPATH . 'wp-admin/includes/class-wp-upgrader.php';
